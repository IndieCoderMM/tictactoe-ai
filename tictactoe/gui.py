...
P2_RECT = pygame.Rect(WINDOW_WIDTH-P2_AVATAR.get_width()-10, 10, PROFILE_SIZE, PROFILE_SIZE)
...
class Gui:
    ...
    def draw_board(self):
        ...
        for row in range(self.board.size):
            for col in range(self.board.size):
                color = YELLOW if self.board.square_name(row, col) in connection else WHITE
    ...
    def display_status(self):
        pygame.draw.circle(self.window, RED, P1_RECT.center, PROFILE_SIZE // 2 + 3)
        pygame.draw.circle(self.window, RED, P2_RECT.center, PROFILE_SIZE // 2 + 3)
        if self.board.is_draw():
            self.display_text("Draw!", 50, 'center', WINDOW_WIDTH - 50)
        elif self.board.winner() == self.board.P1:
            self.display_text("Newbie Wins!", 50, 'center', WINDOW_WIDTH - MARGIN + 10)
        elif self.board.winner() == self.board.P2:
            self.display_text("Batman Wins!", 50, 'center', WINDOW_WIDTH - MARGIN + 10)
        else:
            if self.board.turn == self.board.P1:
                self.display_text("Newbie is thinking...", 20, 'center', WINDOW_WIDTH - MARGIN + 10)
                pygame.draw.circle(self.window, GREEN, P1_RECT.center, PROFILE_SIZE // 2 + 5, 3)
            else:
                self.display_text("Batman is thinking...", 20, 'center', WINDOW_WIDTH - MARGIN + 10)
                pygame.draw.circle(self.window, GREEN, P2_RECT.center, PROFILE_SIZE // 2 + 5, 3)
